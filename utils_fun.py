...
class MetricsUtils:
    ...
    def plot_boundary(self):
        ...
        plot_decision_regions(self.x_train, self.y_train, clf=self.estimator, legend=2)
        plt.scatter(self.x_test[:, 0], self.x_test[:, 1], marker='x', c=self.y_test)  # X ma dwa wymiary
        plt.show()
...
class NotAlone(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        X = check_array(X, accept_sparse=True)
        X_tr = X.sum(axis=1).astype(bool).astype(int).reshape(-1, 1)
        return X_tr
